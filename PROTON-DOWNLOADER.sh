...
registro_file="/userdata/system/.dev/scripts/JCGAMESCLASSICOS/docs/registro.txt"

# Verifica se o arquivo de registro existe
if [ -f "$registro_file" ]; then
    echo "Arquivo de registro encontrado. Executando o script Python..."

    # Execute o script Python incorporado
    python3 - <<EOF
import pygame
import json
import os
        ...
        with open(download_path, 'wb') as file:
            ...
            for chunk in response.iter_content(chunk_size=1024):
                if chunk:
                    ...
                    pygame.draw.rect(screen, (0, 0, 0), (50, screen.get_height() - 80, screen.get_width() - 100, 30))  # Barra de fundo
                    pygame.draw.rect(screen, (0, 255, 0), (50, screen.get_height() - 80, (screen.get_width() - 100) * (percent / 100), 30))  # Barra de progresso
                    pygame.display.flip()  # Atualiza a tela
                    pygame.time.delay(10)  # Delay para atualizar a tela
            print(f"Download concluído: {version_name}")
    else:
        print(f"Erro ao baixar o arquivo: {response.status_code}")
        return

    # O restante da função de extração continua conforme estava
...
EOF
    ...
    echo "Arquivo de registro não encontrado. Fornecendo suporte..."

    # Aqui você pode adicionar o suporte que deseja oferecer, como uma mensagem ou outro tipo de ação.
fi
